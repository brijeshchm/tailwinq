...
function browserResize() {
  ...
  if (k42 == true) {
    if (Number(getStyleValue(document.getElementById("main"), "height").replace("px", "")) > 2200) {
...
function fix_sidemenu() {
  ...
  if (w < 993 && w > 600) {
    if (top == 0) {
      document.getElementById("sidenav").style.top = "77px";
    }
    if (top > 0 && top < 100) {
...
function close_nav(x) {
  ...
  //document.getElementById("leftmenuinnerinner").style.display = "none";
  if (document.getElementById("topnavbtn_" + x)) {
...
(function () {
  ...
  for (i = 0; i < x.length; i++) {
    ...
    if ((x[i].parentNode.tagName == "DIV" || x[i].parentNode.className.indexOf("bar") > -1) && a.substr(0, a.indexOf("/tutorial",1)) ==  b.substr(0, b.indexOf("./tutorial",1))) {
      x[i].className += " active";
    }
  }
  if (window.addEventListener) { 
    document.getElementById("main").addEventListener("click", close_all_nav, true);
    m.addEventListener("click", close_all_topnav, true);
    document.getElementById("right").addEventListener("click", close_all_nav, true);
    document.getElementById("main").addEventListener("wheel", close_all_nav, true);
    document.getElementById("main").addEventListener("touchstart", close_all_nav, true);
  } else if (window.attachEvent) {         
    document.getElementById("main").attachEvent("onclick", close_all_nav);
    m.attachEvent("onclick", close_all_topnav);
    document.getElementById("right").attachEvent("onclick", close_all_nav);
  }
...
function clickFBLike() {
  ...
  document.getElementById("popupDIV").innerHTML = "<iframe src='/fblike.asp?r=" + Math.random() + "' frameborder='no' style='height:200px;width:250px;'></iframe><br><button onclick='hideFBLike()' class='btn black'>Close</button>";
}
function hideFBLike() {
